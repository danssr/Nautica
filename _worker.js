...
// Place this at the VERY TOP of your main Worker JavaScript file.

console.log(`------------------------------------------------------`);
console.log(`WORKER SCRIPT EXECUTION STARTED AT: ${new Date().toISOString()}`);
console.log(`------------------------------------------------------`);

// 1. Check globalThis.SERVICE_NAME directly
console.log(`[LOG] Attempting to read globalThis.SERVICE_NAME...`);
const serviceNameFromGlobalThis = globalThis.SERVICE_NAME;
console.log(`[LOG] Value of globalThis.SERVICE_NAME: >${serviceNameFromGlobalThis}<`); // Delimiters help see undefined/empty
console.log(`[LOG] Type of globalThis.SERVICE_NAME: ${typeof serviceNameFromGlobalThis}`);

// 2. Your actual assignment logic for serviceName
const serviceName = globalThis.SERVICE_NAME || "nautica"; // Or use ?? if "" is valid and desired
console.log(`[LOG] Resolved 'serviceName' variable: >${serviceName}<`);
console.log(`[LOG] Type of 'serviceName' variable: ${typeof serviceName}`);

console.log(`------------------------------------------------------`);
// --- END OF DIAGNOSTIC LOGGING (at top level) ---

// ... (The rest of your existing top-level constants and variable initializations) ...
// For example:
// const rootDomain = globalThis.ROOT_DOMAIN || "example.com";
// const apiKey = globalThis.API_KEY;
// ... etc. ...


// Your main Worker logic (e.g., ES Modules format)
export default {
  async fetch(request, env, ctx) {
    // --- START OF DIAGNOSTIC LOGGING (inside fetch handler) ---
    console.log(`------------------------------------------------------`);
    console.log(`FETCH HANDLER TRIGGERED AT: ${new Date().toISOString()}`);
    const url = new URL(request.url);
    console.log(`[LOG] Request URL: ${url.pathname}${url.search}`);
    console.log(`------------------------------------------------------`);

    // 3. Check env.SERVICE_NAME inside the fetch handler (for ES Modules)
    console.log(`[LOG] Attempting to read env.SERVICE_NAME inside fetch...`);
    const serviceNameFromEnvObject = env.SERVICE_NAME;
    console.log(`[LOG] Value of env.SERVICE_NAME: >${serviceNameFromEnvObject}<`);
    console.log(`[LOG] Type of env.SERVICE_NAME: ${typeof serviceNameFromEnvObject}`);

    const resolvedServiceNameInFetch = env.SERVICE_NAME || "nautica_default_in_fetch";
    console.log(`[LOG] Resolved 'serviceName' in fetch using env object: >${resolvedServiceNameInFetch}<`);

    // You can also log the entire env object to see everything available
    // Be careful with this in production if it contains sensitive data, but for debugging it's useful.
    // console.log('[LOG] Entire env object:', JSON.stringify(env, null, 2));

    console.log(`------------------------------------------------------`);
    // --- END OF DIAGNOSTIC LOGGING (inside fetch handler) ---

    // ... (Your existing fetch handler logic) ...
    // Use the most reliably sourced serviceName for your response:
    const finalServiceNameToUse = serviceNameFromEnvObject || serviceName; // Prefer env object if available

    return new Response(`Hello from service: ${finalServiceNameToUse}! (Top-level 'serviceName' const was: ${serviceName})`);
  }
};

// If using Service Worker format (addEventListener), adapt the fetch handler logging:
/*
addEventListener('fetch', event => {
  // --- START OF DIAGNOSTIC LOGGING (inside fetch listener) ---
  console.log(`------------------------------------------------------`);
  console.log(`FETCH LISTENER TRIGGERED AT: ${new Date().toISOString()}`);
  const url = new URL(event.request.url);
  console.log(`[LOG] Request URL: ${url.pathname}${url.search}`);
  console.log(`------------------------------------------------------`);

  // In Service Worker format, 'env' object is not directly passed.
  // You rely on globalThis for environment variables.
  const serviceNameFromGlobalThisInListener = globalThis.SERVICE_NAME;
  console.log(`[LOG] Value of globalThis.SERVICE_NAME in listener: >${serviceNameFromGlobalThisInListener}<`);

  // The top-level `serviceName` constant would be accessible here too.
  console.log(`[LOG] Top-level 'serviceName' const in listener: >${serviceName}<`);
  console.log(`------------------------------------------------------`);
  // --- END OF DIAGNOSTIC LOGGING (inside fetch listener) ---

  // ... (Your existing event listener logic) ...
  event.respondWith(new Response(`Hello from service: ${serviceName}!`));
});
*/
...
// const accountID = globalThis.CF_ACCOUNT_ID;
// const zoneID = globalThis.CF_ZONE_ID;

// const isApiReady = !!(apiKey && apiEmail && accountID && zoneID);
...
//     document.setTitle(`Welcome to <span class='text-blue-500 font-semibold'>${serviceName}</span>`);
